...
func NewDotfilesTab(win fyne.Window, b *bridge.Bridge) fyne.CanvasObject {
	list := widget.NewList(
		func() int { return 0 },
	...
	refresh := widget.NewButton("Load Dotfiles", func() {
		...
		list.UpdateItem = func(i widget.ListItemID, o fyne.CanvasObject) {
			...
			install.OnTapped = func() {
				go func() {
					err := b.InstallDotfile(entry.Name)
					if err != nil {
						dialog.ShowError(err, win)
					} else {
			...
			update.OnTapped = func() {
				go func() {
					err := b.UpdateDotfile(entry.Name)
					if err != nil {
						dialog.ShowError(err, win)
					} else {
